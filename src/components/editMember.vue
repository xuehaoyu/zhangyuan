<template>
    <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
        <h3 style="display: inline-block">修改员工信息</h3>
        ...
        <el-form-item label="姓名" prop="name">
            <el-input type="tsxt" v-model="ruleForm2.name" auto-complete="off"></el-input>
        </el-form-item>
...
<script>
    export default {
        ...
        created(){
            ...
            console.log(this.id);
            this.ruleForm2.number = this.$route.query.number;
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        var id = that.id;
                        var number=that.ruleForm2.number;
                        var name=that.ruleForm2.name;
                        var phone=that.ruleForm2.phone;
                        fetch("/ajax/member/edit",{
                            method:"post",
                            body:"id="+id+"&number="+number+"&name="+name+"&phone="+phone,
